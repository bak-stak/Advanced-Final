...
modelsummary(
  list(
    ...
    "CDT additive"    = CDT_add_pop,
    "CDT interaction" = CDT_int_pop
  ),
...
The regression table yields few significant results overall. Institutional distrust, however, remains a significant predictor across all tspecifications,and most strongly in the CDT models, in line with previous discover. Nevertheless it is worth noting some exceptions:

-   **Age 35-44**: there is significancy for additive model, indicating that there is a somewhat higher populist tendency, but the relation between distrust and populist vote remain quite stable.

-   **Age 45-54**: this cohort present the most interesting pattern: coefficients get significant once the interaction term is introduced in the computation and it has an opposite sign in the two occurrences. The most plausible explaination for this result is an higher general tendency of this age class to vote for populist parties that, on the contrary, has less to do with distrust compared to the reference class. The CDT interaction plot below illustrates this. Interaction was crucial to capture this tendency, as without it the two effects nullified each other.
...
Regions_model_int_CDT <- glm(abstention
                             ~ Macro_region + Institutional_distrust
                             * age_class,
                             family = binomial(link = "logit"),
                             data = Regions_df_CDT)
...
modelsummary(
  list(
    "Additive Macro-Region control"    = Regions_model_add,
    "Interaction Macro-Region control" = Regions_model_int,
        "Additive Macro-Region control ~ CDT"    = Regions_model_add_CDT,
    "Interaction Macro-Region control ~ CDT" = Regions_model_int_CDT
  ),
  coef_map = coef_labels,
  stars    = TRUE,
  gof_omit = "IC|Log|Adj|Within|Pseudo",
